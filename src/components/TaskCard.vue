<template>
  ...
  >
    <div class="flex justify-between">
      ...
      <button class="deleteButton" @click.prevent="$emit('task-deleted', task.id, task.index)">
        X
      </button>
    </div>
    <div class="flex mt-4 justify-between items-center">
      ...
      <badge v-if="task.priority" :color="badgeColor"
        ><span>{{ task.priority | capitalize }}</span></badge
      >
    </div>
    <div>
      <p
        class="text-gray-700 pt-3 font-semibold font-sans tracking-wide text-sm"
      >
